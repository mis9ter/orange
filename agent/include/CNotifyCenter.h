...
typedef struct NotifyItem {
	NotifyItem(PCSTR _s, DWORD _n, PVOID _p, PFUNC_NOTIFY_CALLBACK _c) {
		dwNotify	= _n;
		pContext	= _p;
		pCallback	= _c;
		cause		= _s;
...
public:
	...
	void			RegisterNotifyCallback(
		PCSTR	pCause,
		WORD wType, WORD wEvent, PVOID pContext, 
		PFUNC_NOTIFY_CALLBACK	pCallback
	) {
		...
		if( m_table.end() == t ) {
			...
			ptr->listener.push_back(std::make_shared<NotifyItem>(pCause, dwNotify, pContext, pCallback));
			{
			...
			t->second->listener.push_back(std::make_shared<NotifyItem>(pCause, dwNotify, pContext, pCallback));
		}
	}
